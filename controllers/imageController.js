...
async function getSessionImagesCount(sessionId) {
  ...
  try {
    ...
    var sql =
      'SELECT count(*) as imagesCount FROM sessionimage WHERE sessionId = ?';
...
async function saveSessionImageToDatabase(sessionId, imagePath, createDt) {
  logger.info([
    'saveSessionImageToDatabase :: START',
    sessionId,
    imagePath,
    createDt,
    '***',
  ...
  try {
    ...
    var sql =
      'INSERT INTO sessionimage (`sessionId`, `image`, `createDt`) VALUES (?, ?, ?)';
    const [results, fields] = await pool.execute(sql, values);
    logger.info(results);
    return results;
  } catch (error) {
...
const handlePost = async (req, res, next) => {
  ...
  if (image) {
    ...
    return res
      .status(5000)
      .json({ message: 'POST canvas image processing error' });
  }
};
