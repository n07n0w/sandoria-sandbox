...
$.widget(".slidein", {
  ...
  _create: function () {
    this.element.addClass(this._class_common).addClass(this._class_panel);
    ...
    this.element
      .mouseleave(function () {
        if (self.options.toClose == "hover" && !this._sliding) {
          self._mouse_out = setTimeout(function () {
            self.open(false);
          }, this._mouse_delay);
        }
    ...
    this._grab_handle
      ...
      .mouseenter(function () {
        if (self.options.toOpen == "hover" && !this._sliding) {
          self._mouse_in = setTimeout(function () {
            self.open(true);
          }, this._mouse_delay);
        }
        clearTimeout(self._mouse_out);
  ...
  _destroy: function () {
    this.element.removeClass(this._class_common).removeClass(this._class_panel);
  ...
  open: function (open) {
    ...
    if (this.options.disabled) {
      // can't open / close a disabled panel (freeze in current state)

      this._trigger("slide", null, {
        open: this.options.open,
        dock: this.options.dock,
      }); // we still trigger to any we didn't slide
    } else {
      ...
      if (this.options.open != open) {
        ...
        var amount = this._is_horizontal()
        ...
        $(this.element).animate(
          ...
          function () {
            self._sliding = false;
            self._trigger("slide", null, {
              open: self.options.open,
              dock: self.options.dock,
            }); // trigger to any "we slid" handlers
          },
        );
      }
    }
  ...
  position: function (position) {
    ...
    this.options.position = Math.max(
      0,
      Math.min(100, this._valid_integer(position)),
    );
    this._reposition();
  },
  ...
  _reposition: function () {
    this._grab_handle.html(
      this.options.prompt ? this.options.prompt : this._default_handle,
    ); // set text before we measure dimensions below
    ...
    this.element.css(
      this.options.dock,
      this.options.open ? 0 : -1 * (size_main - this.options.peek),
    );

    // set handle position in main axis
    this._grab_handle.css(
      this.options.dock,
      this.options.open ? size_main : this.options.peek,
    );

    // put handle in right place in perpendicular axis (never off screen or overlapping)
    var pos_handle =
      (size_other / 100) * this.options.position -
      size_handle / 2 +
      stop_handle;
  ...
  _valid_boolean: function (boolean) {
    ...
    if (jQuery.type(boolean) === "string") {
      if (
        $.inArray(boolean.trim().toLowerCase(), ["0", "false", "no"]) !== -1
      ) {
        boolean = false;
      }
    }
