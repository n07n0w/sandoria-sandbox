...
test('P2P connection module is valid JavaScript', () => {
  ...
  assert.ok(content.includes('function startP2PConnection'), 'Should export startP2PConnection function');
...
test('P2P Global Promise module exists and is valid', () => {
  const p2pPath = path.join(__dirname, '../public/javascripts/p2pConnectionGlobalPromise.js');
  assert.ok(fs.existsSync(p2pPath), 'p2pConnectionGlobalPromise.js should exist');

  const content = fs.readFileSync(p2pPath, 'utf8');
  assert.ok(content.includes('startP2PConnection'), 'Should define startP2PConnection');
  assert.ok(content.includes('startP2PConnectionAsync'), 'Should define startP2PConnectionAsync');
  assert.ok(content.includes('setupWebSocket'), 'Should have setupWebSocket function');
  assert.ok(content.includes('setupPeerConnection'), 'Should have setupPeerConnection function');
});
...
test('P2P module has reconnection logic', () => {
  ...
  assert.ok(content.includes('setTimeout'), 'Should use setTimeout for reconnection');
});
...
test('P2P module has message queuing', () => {
  ...
  assert.ok(content.includes('pendingMessages'), 'Should have pendingMessages queue');
