...
test('Core modules can be loaded without errors', async () => {
  ...
  assert.ok(fs.existsSync(path.join(__dirname, '../dbConfig.js')), 'dbConfig.js should exist');
});
...
test('Constants are properly defined', async () => {
  ...
  assert.strictEqual(typeof constants.BASE_URL, 'string', 'BASE_URL should be a string');
});
...
test('Repositories can be instantiated', async () => {
  ...
  assert.ok(fs.existsSync(path.join(__dirname, '../repository/sandboxRepository.js')),
    'sandboxRepository.js should exist');
  assert.ok(fs.existsSync(path.join(__dirname, '../repository/categoryRepository.js')),
...
test('UUID generation works', async () => {
  ...
  assert.match(uuid1, /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i, 'UUID should have valid format');
});
...
test('WebSocket module loads', async () => {
  ...
  assert.strictEqual(typeof WebSocket, 'function', 'WebSocket should be a constructor');
});
...
test('Express middleware is configured', async () => {
  ...
  assert.ok(app.locals.constants, 'constants should be available in app.locals');
});
